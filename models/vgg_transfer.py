        ...
        for phase in ["train", "val"]:
            ...
            F1 = (
                ...
                * computed_metrics["recall"]
                / (computed_metrics["precision"] + computed_metrics["recall"])
            ).numpy()
            F1[np.isnan(F1)] = 0
            weighted_F1 = np.sum(F1 * weights.numpy())

            print(
                "{} Loss: {:.4f}, Acc: {:.4f}, Weighted F1: {:.4f}".format(
                    phase, epoch_loss, computed_metrics["acc"], weighted_F1
                )
            )
...
if __name__ == "__main__":
    ...
    optimizer = torch.optim.SGD(
        params_to_update, lr=params.lr, momentum=params.momentum
    )
    ...
    test_ds = ClassificationDataset(
        root_dir=root_dir, train=False, transform=test_transformations
    )
    ...
    model_ft, hist = train_model(
        model, dataloaders_dict, criterion, optimizer, metrics, num_epochs=num_epochs
    )
